  ...
  test('should validate profile GET and PUT requests', async ({ page }) => {
    ...
    await test.step('Check profile GET response [no-screenshot]', async () => {
      ...
      if (getRequest?.responseBody) {
        ...
        await api.saveApiResponse(getRequest.responseBody, getRequest.url, 'GET', path.join(resultsDir, 'step-02-profile-get-response.json'));
      } else {
    ...
    await test.step('Change profile fields (not email)', async () => {
      ...
      console.log(`✅ Step 4: Profile updated with ID ${testRunId}, random text: ${randomString}`);
    });
    ...
    await test.step('Check profile PUT and GET response [no-screenshot]', async () => {
      ...
      if (putRequest?.responseBody) {
        ...
        await api.saveApiResponse(putRequest.responseBody, putRequest.url, 'PUT', path.join(resultsDir, 'step-06-profile-put-response.json'));
      } else {
