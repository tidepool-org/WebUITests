...
const BasicsPage_1 = __importDefault(require("@pom/patient/BasicsPage"));
const DailyPage_1 = __importDefault(require("@pom/patient/DailyPage"));
base_1.test.describe('Patient Data Navigation and Visualization', () => {
    base_1.test.beforeEach(async ({ page }) => {
        await base_1.test.step('Given user has been logged in', async () => {
            const basicsPage = new BasicsPage_1.default(page);
    ...
    (0, base_1.test)('should display daily chart when selecting a date from basics page', async ({ page }) => {
        const basicsPage = new BasicsPage_1.default(page);
        const dailyPage = new DailyPage_1.default(page);
        let selectedDateText;
        await base_1.test.step('When the navigation bar is visible', async () => {
            await basicsPage.navigationBar.buttons.viewData.waitFor({
                state: 'visible',
            });
        });
        await base_1.test.step('When the user clicks on the most recent day', async () => {
        ...
        await base_1.test.step('Then the daily chart is visible and correctly rendered', async () => {
            ...
            await (0, base_1.expect)(chartContainer).toHaveScreenshot('daily-chart-1.png');
        });
    });
    // Bolus dashboard functionality
    (0, base_1.test)('should display bolus dashboard when selecting a date from basics page', async ({ page, }) => {
        const basicsPage = new BasicsPage_1.default(page);
        const dailyPage = new DailyPage_1.default(page);
        let selectedDateText;
        await base_1.test.step('When the navigation bar is visible', async () => {
        ...
        await base_1.test.step('Then the daily chart is visible and correctly rendered', async () => {
            ...
            // await expect(dailyPage.navigationSubMenu.currentDate).toContainText(selectedDateText);
            // Capture chart screenshot for visual regression
            await (0, base_1.expect)(chartContainer).toHaveScreenshot('daily-chart-2.png');
        });
    });
    // Infusion Site Changes dashboard functionality
    (0, base_1.test)('should display Infusion site changes dashboard when selecting a date from basics page', async ({ page, }) => {
        const basicsPage = new BasicsPage_1.default(page);
        const dailyPage = new DailyPage_1.default(page);
        let selectedDateText;
        await base_1.test.step('When the infusion site changes dashboard is visible', async () => {
        ...
        await base_1.test.step('When testing Fill Cannula functionality', async () => {
            // Verify radio button options
            await basicsPage.tubingPrimeSection.settingsOption.fillCannula.waitFor({
                state: 'visible',
                timeout: 60000,
            });
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.settingsOption.fillCannula).toBeVisible();
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.settingsOption.fillTubing).toBeVisible();
            ...
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.cannulaIcons).toBeAttached();
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.tubingIcons).not.toBeAttached();
        ...
        await base_1.test.step('Then the daily chart shows correct cannula fill date', async () => {
            const chartContainer = dailyPage.dailyChart.container;
            await chartContainer.waitFor({ state: 'visible' });
            if (!selectedDateText) {
                throw new Error('Selected date text is null');
            ...
            // await expect(dailyPage.navigationSubMenu.currentDate).toContainText(selectedDateText);
            await (0, base_1.expect)(chartContainer).toHaveScreenshot('daily-chart-cannula.png');
        });
        // Return to basics page and test Fill Tubing Option
        await base_1.test.step('When testing Fill Tubing functionality', async () => {
            ...
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.tubingIcons).toBeAttached();
            await (0, base_1.expect)(basicsPage.tubingPrimeSection.cannulaIcons).not.toBeAttached();
        ...
        await base_1.test.step('Then the daily chart shows correct tubing fill date', async () => {
            const chartContainer = dailyPage.dailyChart.container;
            await chartContainer.waitFor({ state: 'visible' });
            if (!selectedDateText) {
                throw new Error('Selected date text is null');
            ...
            // await expect(dailyPage.navigationSubMenu.currentDate).toContainText(selectedDateText);
            await (0, base_1.expect)(chartContainer).toHaveScreenshot('daily-chart-tubing.png');
        });
    });
    // TODO: Previous test doesn't test values. Should we? :)
    // Readings in range functionality
    (0, base_1.test)('The hover over elements in sidebar shows correct values', async ({ page }) => {
        // Stats for BGM
        ...
        for (let i = 0; i < 5; i += 1) {
            ...
            await base_1.test.step('When the user hovers over the Avg. Daily Readings In Range chart', async () => {
                await bar.hover();
            });
            await base_1.test.step('Then the correct header is visible', async () => {
                await base_1.expect
                    .soft(basicsPage.statsSidebar.readingsInRange.header)
                    .toContainText(expectedHeadersReadingInRange[i].header);
            });
            await base_1.test.step('Then the correct value is visible', async () => {
                await base_1.expect
                    .soft(barLabel)
        ...
        for (let i = 0; i < expectedHeadersTimeInRange.length; i += 1) {
            ...
            await base_1.test.step('When the user hovers over the Avg. Daily Time In Range chart', async () => {
                await bar.hover();
            });
            await base_1.test.step('Then the correct header is visible', async () => {
                await base_1.expect
            ...
            await base_1.test.step('Then the correct value is visible', async () => {
                await base_1.expect.soft(barLabel).toContainText(expectedHeadersTimeInRange[i].value.toString());
            });
        }
    });
    // Other CGM tooltip functionality
    (0, base_1.test)('other CGM tooltip functionality', async ({ page }) => {
        const basicsPage = new BasicsPage_1.default(page);
        await basicsPage.statsSidebar.toggleTo('CGM');
        const expectedHeadersTimeInRange = [
        ...
        for (let i = 0; i < expectedHeadersTimeInRange.length; i += 1) {
            ...
            await base_1.test.step('When the user hovers over the Avg. Daily Total Insulin chart', async () => {
                await bar.hover();
            });
            await base_1.test.step('Then the correct header is visible', async () => {
                await base_1.expect
