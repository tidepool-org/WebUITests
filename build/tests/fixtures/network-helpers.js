...
exports.createNetworkHelper = createNetworkHelper;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const endpoint_registry_1 = require("../../endpoint-schema/endpoint-registry");
const ENDPOINTS = {
...
class NetworkHelper {
    ...
    async waitForEndpoint(endpointName, method, timeout = 30000) {
        const pattern = ENDPOINTS[endpointName];
        if (!pattern) {
            throw new Error(`Unknown endpoint: ${endpointName}`);
        }
        const startTime = Date.now();
        while (Date.now() - startTime < timeout) {
            const matches = this.captures.filter(capture => pattern.test(capture.url) && capture.method.toLowerCase() === method.toLowerCase());
            if (matches.length > 0) {
     ...
     */
    getCapturesForEndpoint(endpointName) {
        const pattern = ENDPOINTS[endpointName];
        if (!pattern) {
            throw new Error(`Unknown endpoint: ${endpointName}`);
        }
     ...
     */
    async saveApiResponse(response, endpoint, method, fileName, testInfo) {
        const responseData = {
            _request: {
                method,
                endpoint,
     ...
     */
    async validateEndpointResponse(endpointName) {
        const schema = (0, endpoint_registry_1.getEndpointSchema)(endpointName);
        ...
        if (request?.responseBody) {
            ...
            if (stepCounterObj) {
                ...
                await this.saveApiResponse(request.responseBody, request.url, schema.method, fileName, globalThis.testInfo);
            }
        }
        return request;
    }
     ...
     */
    async saveForDependentTests(endpointName, testName) {
        const schema = (0, endpoint_registry_1.getEndpointSchema)(endpointName);
        const capture = this.getLatestCaptureMatching(schema.method, schema.url);
        if (capture) {
     ...
     */
    loadFromProducerTest(testName) {
        const filePath = path.join(process.cwd(), 'log', 'test-data-pipeline', `${testName}-response.json`);
        ...
        throw new Error(`Producer test data not found at: ${filePath}. Please run ${testName} test first.`);
    }
    /**
     ...
     */
    validateDataConsistency(producerCapture, consumerCapture, fieldsToValidate, requiredFields = ['fullName']) {
            ...
            if (producerValue !== undefined && producerValue !== null) {
                ...
                if (Array.isArray(producerValue) && Array.isArray(consumerValue)) {
                    if (JSON.stringify(producerValue) !== JSON.stringify(consumerValue)) {
                        throw new Error(`${fieldPath} mismatch - Expected: ${JSON.stringify(producerValue)}, Got: ${JSON.stringify(consumerValue)}`);
                    }
                }
                else if (producerValue !== consumerValue) {
                    throw new Error(`${fieldPath} mismatch - Expected: ${producerValue}, Got: ${consumerValue}`);
                }
            }
     ...
     */
    async validateProducerConsumerData(producerEndpointName, consumerEndpointName, fieldsToValidate) {
        const producerSchema = (0, endpoint_registry_1.getEndpointSchema)(producerEndpointName);
        const consumerSchema = (0, endpoint_registry_1.getEndpointSchema)(consumerEndpointName);
        ...
        const producerCapture = this.getLatestCaptureMatching(producerSchema.method, producerSchema.url);
        const consumerCapture = this.getLatestCaptureMatching(consumerSchema.method, consumerSchema.url);
        if (!producerCapture) {
     ...
     * @returns The captured network request or null if not found
     */
    validateEndpointResponseSilent(endpointName) {
        const schema = (0, endpoint_registry_1.getEndpointSchema)(endpointName);
        const request = this.getLatestCaptureMatching(schema.method, schema.url);
        return request;
    }
     ...
     * @param fieldsToValidate - Optional array of fields to validate (overrides endpoint schema)
     */
    async compareEndpointResponse(consumerEndpointName, producerCapture, fieldsToValidate) {
        ...
        if (stepCounterObj) {
            ...
            const { testInfo } = globalThis;
            await this.saveApiResponse(comparisonData, consumerCapture.url, consumerCapture.method, fileName, testInfo);
        }
...
function createNetworkHelper(page) {
