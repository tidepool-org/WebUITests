 ...
 */
async function setupPatientSession(page) {
    // Wait for patient navigation to be available
    const nav = new PatientNavigation_1.default(page);
    await Promise.all([
 ...
 */
async function isInPatientContext(nav, page) {
    try {
 ...
 */
async function getCurrentPageState(nav, page) {
    const url = page.url();
...
const navigationStrategies = {
    ...
    default: [
        ...
        },
        {
            ...
            action: async (state) => {
                ...
                await state.page.goto(`${env_1.default.BASE_URL}/data`);
    ...
    Profile: [
        ...
        },
        {
            ...
            condition: async (state) => state.page.url().includes('/profile') &&
                (await state.page
            ...
            action: async (state) => {
                ...
                await state.page.goto(env_1.default.BASE_URL);
 ...
 */
async function executeNavigationStrategy(state) {
...
async function navigateTo(targetPage, page) {
    ...
    if (!success) {
        ...
        if (targetPage === 'Profile') {
            ...
            try {
                let fallbackURL = env_1.default.BASE_URL;
                // For sub-pages that might not be available, fall back to the main page
                if (targetPage === 'ShareData') {
                    fallbackURL = `${env_1.default.BASE_URL}/share`; // Fall back to main Share page
                }
                else if (targetPage === 'ProfileEdit') {
                    fallbackURL = `${env_1.default.BASE_URL}/profile`; // Fall back to main Profile page
                }
                else if (['Basics', 'Daily', 'BGLog', 'Trends', 'Devices'].includes(targetPage)) {
                    fallbackURL = `${env_1.default.BASE_URL}/data`; // Fall back to main ViewData page
                }
                else if (nav.pages[targetPage].verifyURL) {
                    fallbackURL = `${env_1.default.BASE_URL}/${nav.pages[targetPage].verifyURL}`;
                }
