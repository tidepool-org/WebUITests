...
class ProfilePage {
    ...
    async fillField(field, value) {
        ...
        if (!locator)
            throw new Error(`No locator defined for field: ${field}`);
    ...
    async selectDiagnosisType(index) {
    ...
    async getCurrentDiagnosisIndex() {
        const diagnosisCombo = this.page.getByRole('combobox', { name: 'Diagnosed as' });
        if (await diagnosisCombo.isVisible({ timeout: 3000 })) {
    ...
    async fillDiagnosisDate(date) {
        return this.fillField('diagnosisDate', date);
    }
    async fillClinicalNotes(notes) {
    ...
    async saveProfile() {
        ...
        const saveProfilePromise = this.page.waitForResponse(response => response.url().includes('/metadata/') &&
     ...
     */
    async editButtonDisplays(shouldBeVisible) {
