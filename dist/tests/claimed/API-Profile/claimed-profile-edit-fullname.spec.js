...
test.describe('Claimed Account Settings edit (Full Name only) updates Profile endpoint and visually updates for user, clinic, and shared member', () => {
    ...
    test('should allow navigation to account settings, edit full name, and verify profile update for claimed, shared, and clinician users', {
        ...
        await test.stepNoScreenshot('Then profile endpoint responds with GET request consistent with schema', async () => {
        ...
        await test.stepNoScreenshot('Then PUT request is validated and name is set to new value', async () => {
        ...
        await test.stepNoScreenshot('Then GET request matches the saved PUT request', async () => {
            await api.validateEndpointResponse('profile-metadata-get');
        ...
        await test.stepNoScreenshot('Then shared user sees view-only claimed profile data with matching data', async () => {
        ...
        await test.stepNoScreenshot('Then clinician sees claimed profile data with matching data and no save access', async () => {
            await api.compareEndpointResponse('profile-metadata-get', putCapture);
        });
    });
});
